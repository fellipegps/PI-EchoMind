...
# Rota para buscar um FAQ pelo ID
...
# Rota para atualizar um FAQ
@router.put("/{faq_id}", response_model=FAQResponse)
async def update_faq(faq_id: UUID, faq_in: FAQUpdate, db: AsyncSession = Depends(get_db)):
    faq = await FAQService.update_faq(db, faq_id, faq_in)
    if not faq:
        raise HTTPException(status_code=404, detail="FAQ não encontrada")
    return faq

# Rota para deletar um FAQ
@router.delete("/{faq_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_faq(faq_id: UUID, db: AsyncSession = Depends(get_db)):
    deleted = await FAQService.delete_faq(db, faq_id)
    if not deleted:
        raise HTTPException(status_code=404, detail="FAQ não encontrada")
