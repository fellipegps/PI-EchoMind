...
def test_rls_logging_failure_does_not_reference_request_tenant(monkeypatch) -> None:
    ...
    class FailingEngine:
        def begin(self):
            raise PermissionError("detalhe sensivel do banco")

    monkeypatch.setattr(rag_engine, "DATABASE_URL", "postgresql://local/test")
    monkeypatch.setattr(rag_engine, "engine", FailingEngine())
    monkeypatch.setattr(
        rag_engine,
        "emit_event",
    ...
    try:
        rag_engine._enable_langchain_rls_if_possible()
    finally:
        rag_engine._enable_langchain_rls_if_possible.cache_clear()
